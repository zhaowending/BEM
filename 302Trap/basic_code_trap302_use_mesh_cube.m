...
slot_W=100*um; % the size of loading slot
%%%%%%% two compensation electrodes on the both side of slot, in the middle  of RFs %%%%%%%%%
downdc_W=20*um; updc_W=20*um;
dc_L=1000*um;
%%%%%%%%%%%%%%% two rf's size %%%%%%%%%%%%%%%%%%%%%%%%
downrf_W=60*um; uprf_W=60*um; rf_L=1500*um ; % size of rf electrodes
gap_Y=7*um; gap_X=7*um;  % gapY is the gap between DC and RF, gapX is the gap between DC electrodes
%%%%%%%%%%%% for DC segments %%%%%%%%%%%%%%%%%%%%%%%%%%%
DC_Y=200*um; DC_X=70*um; % the length and width of DC electrode
...
out_length=1000*um; %  the length of
left_points=[-719.6, -522.12, -365.26, 189.44, 375.66, 532.9]*um;
right_points=[-532.9, -375.66, -189.44, 365.26, 522.12, 719.6]*um;
num_DC_pair=6;  % the number of DC segment electrodes pairs (in center) you want to build
use_pair=[2:1:6];  % the DC segement electrodes pairs (in center) you will use in real
...
Filename='mesh_surface302_4.json';
%% doing RF meshing
if (Switch_new_mesh==0)
    ...
    [DC_x,DC_y,en]=design_trap302_DCsegment(slot_W,rf_L,num_DC_pair,use_pair,gap_Y,uprf_W,downrf_W,gap_X,DC_X,DC_Y,updc_W,downdc_W,Z_surface,Z_depth,dc_L,right_points,left_points,out_length);
    [T_array,node_array,T_num_electrode,T_total,T_pos1,T_pos2] = mesh_trap302_DC_segement(T_array,node_array,T_num_electrode,T_total,T_pos1,T_pos2,DC_x,DC_y,en,mesh_divide,plot_switch,figure_all_plot,Z_depth,Z_surface,um);
    [T_array,node_array,T_num_electrode,T_total,T_pos1,T_pos2] = mesh_trap302_DC_mid(T_array,node_array,T_num_electrode,T_total,T_pos1,T_pos2,DC_x,DC_y,en,mesh_divide,plot_switch,figure_all_plot,Z_depth,Z_surface,um);
...
%Filename='charge_surface302_4.mat';
Filename='charge_surface302_4.json';
...
Switch_new_potential=0;  %0:doing a new one,  1: loading a exist
%Filename='potential_surface302_4_more1.mat';
Filename='potential_surface302_center.json';
...
if (num_total_electrode==ntmp(2))&(T_total==ntmp(1))
    ...
    if (Switch_new_potential==0)
        xr=[0*um,80*um,50];
        yr=[-30*um,30*um,30];
        zr=[60*um,120*um,70]; % xy0: doing a new chargebasis calculationz_range=[begin,  end ,  number]

function  [y_plot,z_plot,potential_plot] = calplot_yzplane_real_potential(plot_switch,yr_plot,zr_plot,plot_density,xr,yr,zr,x_want,nx,ny,nz,points,static_potential)
...
if (x_want<x_line(1))|(x_want>x_line(nx))
    ...
    for j=1:nx
        if (x_line(j)==x_want)
            ...
            if (x_line(j)<x_want)&(x_line(j+1)>x_want)
                x_begin=j; x_end=j+1;
                break
            end
        end

...
%% voltage setting
Vrf=300;
V_dcsegement=[1.85,3.98,-2.76,-1.27,-1.9,-1.1,-1.9,-1.1,-2.76,-1.27,1.85,3.98,0.52,-0.48];
...
xr_plot=[-15*um,15*um,40];
yr_plot=[-15*um,15*um,40];
zr_plot=[80*um,130*um,70];
...
% use fitting method to calculate trap freuqncy 
%[center_fit_2,H_matrix_fit_2,omega3_2,frequency3_2,c_H_2,H_fit_V_2,H_fit_2] = calculate_trap_freq_byfitting_NOfirst(center_pseudo,x,y,z,static_matrix,pseudo2_matrix,um,q,mass)

% use fitting method to calculate trap frequency 
% This  function, calculate_trap_freq_byfitting, IS WRONG
...
title_txt='The All pseudo potential field on YZ plane around center '; 
[y_plot,z_plot,potential_plot,center_pseudo_back2,pseudo_PPdepth_center] = Plot_YZ_pseudo_way2(plot_switch,plot_density,xr_plot,yr_plot,zr_plot,x_want,x,y,z,pseudo2_matrix,title_txt);
...
x_want=center_pseudo(1);
plot_switch=0; plot_density=10;
title_txt='The All static potential field on YZ plane around center '; 
...
yr_plot=[-10*um+center_pseudo(2),10*um+center_pseudo(2),8];
zr_plot=[-10*um+center_pseudo(3),10*um+center_pseudo(3),8];
% the number of ny nz should be even number like 20,24
y_want=center_pseudo(2); z_want=center_pseudo(3);
plot_switch=0;
[c_2order,center_fit_xaxis_2order] = plot_Xais_arond_trap_center(plot_switch,center_pseudo,xr_plot,yr_plot,zr_plot,x,y,z,pseudo2_matrix);
...
fprintf('The center point is at [%f um, %f  um,%f  um]\n ',center_pseudo(1)/um,center_pseudo(2)/um,center_pseudo(3)/um);
